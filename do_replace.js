...
let code = fs.readFileSync('overview.txt', 'utf8');

const newHeader = `import React, { useEffect, useState } from "react";
...
const endIndex = code.indexOf('    if (!config) {\n      // Configuration not found');

if (startIndex !== -1 && endIndex !== -1) {
  let rest = code.substring(endIndex);
  // Remove "    if (!config) {\n      // Configuration not found (meaning they haven't configured it yet)\n      return ("
  let bracketIndex = rest.indexOf('return (\n');
  rest = rest.substring(bracketIndex + 9);
  
  // Now find where that return block ends.
  // We can just find the end of the NOT_CONFIGURED return block and then "const config = persistedConfig"
  let endOfIfBlock = rest.indexOf('  // If a valid config exists, show the summary');
  if (endOfIfBlock === -1) {
    endOfIfBlock = rest.indexOf('  return (\n    <div className="w-full max-w-5xl');
  }

  let finalRest = rest.substring(endOfIfBlock);
  finalRest = finalRest.replace("{showSavedSuccess && (", `{showSavedSuccess && (
...
`;
  ...
  fs.writeFileSync('src/components/digital-twin/configuration-overview.tsx', newHeader + newMiddle + finalRest);
  console.log("Replaced configuration-overview successfully");
} else {
