...
// Find the start of the final return block for the configured UI
const regex = /return \(\s*<div className="w-full max-w-5xl mx-auto flex flex-col mt-8 pb-32">/g;
let match;
...
if (startIndex !== -1 && lastReturnIndex !== -1) {
  let rest = code.substring(lastReturnIndex);
  
  rest = rest.replace("{showSavedSuccess && (", `{showSavedSuccess && (
  ...
  fs.writeFileSync('src/components/digital-twin/configuration-overview.tsx', newHeader + newMiddle + rest);
