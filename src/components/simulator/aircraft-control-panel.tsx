...
const statusClass = (fault: boolean, warning: boolean) => fault ? "text-rose-400" : warning ? "text-amber-300" : "text-neutral-300";
...
export function AircraftControlPanel(props: Props) {
  ...
  const fault = props.motorHealths.some((value) => value <= 0.01);
  const warning = !fault && (props.motorOverrides.some((value) => value < 0.99) || props.telemetry.batteryLevel < 20);
  const explanation = fault
    ? "A failed motor produces no thrust. The asymmetric thrust is passed directly into the flight physics model, so attitude and power telemetry show the resulting response."
    : props.motorOverrides.some((value) => value < 0.99)
      ? `A reduced motor command creates asymmetric thrust. Current attitude: roll ${(safe(props.telemetry.rotation.roll) * 180 / Math.PI).toFixed(1)}°, pitch ${(safe(props.telemetry.rotation.pitch) * 180 / Math.PI).toFixed(1)}°.`
      : props.payloadKg > 0
        ? `Payload raises simulated mass to ${(safe(props.telemetry.totalMassKg ?? 0)).toFixed(2)} kg; altitude hold must command more thrust to maintain hover.`
  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 w-80 max-w-[86vw] bg-neutral-950/95 border-r border-white/10 p-4 text-white font-mono overflow-y-auto transition-transform lg:relative lg:inset-auto lg:z-auto lg:w-full lg:max-w-none lg:bg-transparent ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
      <div className="flex items-center justify-between border-b border-white/10 pb-3"><div><p className="text-[10px] tracking-[.2em] text-white/45">AIRCRAFT</p><h2 className="font-bold text-sm">{props.droneName}</h2></div><span className={`text-[10px] font-bold uppercase ${statusClass(fault, warning)}`}>{fault ? "● Fault" : warning ? "● Warning" : "● Stable"}</span></div>
      ...
      <Section title="POWER"><div className="flex items-center justify-between text-xs"><span><Battery className="mr-1 inline h-3 w-3"/>Battery</span><b>{safe(props.telemetry.batteryLevel).toFixed(0)}%</b></div><input aria-label="Battery state" className="mt-2 w-full accent-[#FF5500]" type="range" min="0" max="100" step="5" value={Math.round(safe(props.telemetry.batteryLevel, 100) / 5) * 5} onChange={e => props.onBattery(Number(e.target.value))}/><div className="mt-2 grid grid-cols-3 text-[10px] text-white/55"><span>{safe(props.telemetry.batteryVoltage ?? 0).toFixed(1)} V</span><span>{safe(props.telemetry.batteryCurrentAmps ?? 0).toFixed(1)} A</span><span>{safe(props.telemetry.batteryPowerWatts ?? 0).toFixed(0)} W</span></div></Section>
      ...
      <Section title="SYSTEM">{(["GPS", "IMU", "COMPASS", "FLIGHT CONTROLLER"] as const).map(label => <div key={label} className="flex justify-between py-0.5 text-[10px]"><span>{label}</span><span className={label === "FLIGHT CONTROLLER" || props.sensors[label.toLowerCase() as keyof typeof props.sensors] ? "text-neutral-300" : "text-rose-400"}>{label === "FLIGHT CONTROLLER" || props.sensors[label.toLowerCase() as keyof typeof props.sensors] ? "ONLINE" : "OFFLINE"}</span></div>)}</Section>
